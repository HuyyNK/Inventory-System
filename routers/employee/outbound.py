...
router = APIRouter(prefix="/employee/outbound", tags=["employee_outbound"])
...
async def add_outbound_page(request: Request, user: dict = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("employee/outbound/add.html", {
        "request": request,
        "user": user,
        "products": products,
...
async def add_product_row(request: Request, index: int, user: dict = Depends(get_current_user)):
    products = get_products_list()
    return templates.TemplateResponse("employee/outbound/product_row.html", {
        "request": request,
        "index": index,
        "products": products,
        "today": datetime.now().date().isoformat()
...
async def list_outbounds_page(request: Request, user: dict = Depends(get_current_user)):
    return templates.TemplateResponse("employee/outbound/list.html", {"request": request, "user": user})
...
async def detail_outbound_page(request: Request, outbound_id: int, user: dict = Depends(get_current_user)):
    try:
        ...
        return templates.TemplateResponse("employee/outbound/detail.html", {
            "request": request,
            "user": user,
            "outbound": outbound,
