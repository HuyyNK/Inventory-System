...
router = APIRouter(prefix="/employee/inbound", tags=["employee_inbound"])
...
async def add_inbound_page(request: Request, user: dict = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("employee/inbound/add.html", {
        "request": request,
        "user": user,
        "suppliers": suppliers,
        "products": products,
...
async def add_product_row(request: Request, index: int, supplier_id: int = None, user: dict = Depends(get_current_user)):
    ...
    return templates.TemplateResponse("employee/inbound/product_row.html", {
        "request": request,
        "index": index,
        "products": products,
        "today": datetime.now().date().isoformat()
...
async def list_inbounds_page(request: Request, user: dict = Depends(get_current_user)):
    return templates.TemplateResponse("employee/inbound/list.html", {"request": request, "user": user})
...
async def detail_inbound_page(request: Request, inbound_id: int, user: dict = Depends(get_current_user)):
    try:
        ...
        return templates.TemplateResponse("employee/inbound/detail.html", {
            "request": request,
            "user": user,
            "inbound": inbound,
